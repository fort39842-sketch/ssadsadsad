...
const TypingRace = ({ paragraph, onComplete }: TypingRaceProps) => {
  ...
  const getCharColor = (index: number) => {
    ...
    if (userChar === correctChar) return "text-neon-green";
      ...
      <div className="w-full max-w-4xl space-y-6 animate-in fade-in duration-500">
        <div className="text-center space-y-2">
          <div className="flex items-center justify-center gap-2">
            <Zap className="w-8 h-8 text-neon-cyan text-glow-cyan animate-pulse" data-testid="icon-zap" />
            <h2 className="text-3xl md:text-4xl font-bold text-glow-purple" data-testid="heading-type-fast">
              Type Fast!
            </h2>
          </div>
        ...
        <div className="relative">
          ...
          <div className="relative bg-card border border-border rounded-lg overflow-hidden">
            <div className="h-2 bg-muted">
              <div 
                className="h-full bg-gradient-to-r from-neon-purple to-neon-green transition-all duration-300"
            ...
            <div className="p-6 md:p-8 space-y-6">
              <div className="bg-muted rounded-lg p-6 font-mono text-lg leading-relaxed select-none" data-testid="text-paragraph">
              ...
              <textarea
                ...
                onPaste={handlePaste}
                className="w-full h-48 bg-input border border-border rounded-lg p-6 font-mono text-lg leading-relaxed resize-none focus:border-neon-cyan focus:ring-2 focus:ring-neon-cyan/50 transition-all"
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground space-y-1">
                  <div data-testid="text-progress">Progress: {Math.round(progress)}%</div>
                  <div data-testid="text-accuracy">Accuracy: {calculateAccuracy().toFixed(1)}%</div>
                </div>

                <Button
                  onClick={handleFinish}
                  disabled={!isComplete}
                  className="bg-gradient-to-r from-neon-purple to-neon-cyan hover:opacity-90 text-primary-foreground font-bold px-8 py-6 text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                  data-testid="button-finish-race"
                >
                  Finish Race
                </Button>
              </div>
        ...
        <div className="text-center text-sm text-muted-foreground" data-testid="text-warning">
